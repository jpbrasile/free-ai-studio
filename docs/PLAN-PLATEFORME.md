...
| **P2** Gateway | La passerelle existe : chaîne `gemini → openrouter → groq`, bascule annoncée et non silencieuse, pause jusqu'à minuit Pacifique. Ce qui manque : elle **ne lit aucun registre**, et elle **ne compte rien d'avance** | `free-tier-manager/app.py:143` (`PROVIDERS`), `:248` (`LIMITES_PUBLIEES`), `:764` (`apply_rate_limit_cooldown`) | partiel — **et sur un point le réel réfute le plan** (ci-dessous) | en partie |
