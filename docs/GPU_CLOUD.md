...
- Kaggle = fallback automatisable pour les seuls jobs GPU, **et** accès direct cliquable ;
...
- La [politique d'usage](https://www.kaggle.com/aup) interdit d'abuser des ressources offertes, entre autres pour du « server farming » ou une activité sans rapport avec la science des données et l'apprentissage automatique. Conséquence, décidée le 11/09/2026 : le mode `auto` du Sandbox n'envoie sur Kaggle que les jobs `gpu=true`, et seulement quand Modal et le worker local sont indisponibles ; un job sans GPU passe au notebook Colab (`cpu_job_not_sent` dans `fallback_attempts`). Un job lancé explicitement avec `provider=kaggle` reste votre choix : réservez-le aux calculs de données ou d'apprentissage.
